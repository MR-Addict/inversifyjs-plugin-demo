...
// dist/plugins/dashboard.plugin.js
var plugin = {
  meta: {
    id: "dashboard-plugin",
    name: "Dashboard Plugin",
    description: "Listens to host events and shared state so it can build a live activity feed."
  },
  activate(ctx) {
    const panel = ctx.ui.createPanel({
      id: "dashboard-panel",
      title: "Dashboard Plugin",
      description: "This plugin uses ctx.events, ctx.state, ctx.commands, and plain DOM APIs to build a live dashboard."
    });
    const doc = panel.body.ownerDocument;
    const intro = doc.createElement("p");
    intro.textContent = "Load the greeter plugin and send a few greetings. This dashboard subscribes to the host event bus and reacts in real time.";
    ...
    const syncTotal = (total) => {
      ...
      panel.setStatus(total === 0 ? "Waiting for the first greeting." : `Observed ${total} greetings.`);
    };
    const appendEvent = (payload, source, when) => {
    ...
    const totalSubscription = ctx.state.subscribe(GREETING_COUNT_KEY, syncTotal);
    const eventSubscription = ctx.events.subscribe(GREETING_EVENT, (event) => {
      appendEvent(event.payload, event.source, event.occurredAt);
      ctx.logger.info(`${plugin.meta.name} observed greeting #${event.payload.total}.`);
    });
...
var dashboard_plugin_default = plugin;
export {
  dashboard_plugin_default as default
};
